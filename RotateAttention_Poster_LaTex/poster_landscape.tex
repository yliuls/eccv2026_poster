...
\begin{poster}{
 ...
 % Eye Catcher
 {
  \parbox[c]{0.22\linewidth}{\raggedright\large\bf\color{blue} ECCV 2026\\[0.2em] Submission \#1552}
 }
 % Title
...
\headerbox{\bf\color{blue} Problem Definition \& Contribution}{name=problem,column=0,row=0,span=2}{
    ...
    \begin{itemize}\compresslist
        ...
        \item \textbf{Key finding: rotation matrices must be orthogonal}; unconstrained learned rotations amplify quantization error.
...
\headerbox{\bf\color{blue} Method: RoPE-aware Rotation + Range-optimized $\mathbf{P}$}{name=method,column=2,row=0,span=3}{
    ...
    \begin{minipage}[t]{0.37\linewidth}
        \centering
        \raisebox{-\height}{\includegraphics[width=\linewidth]{rotation}}
    \end{minipage}
}
...
\headerbox{\bf\color{blue} Outlier Analysis: What 3D RoPE Does to $\mathbf{Q}$/$\mathbf{K}$}{name=findings,column=0,below=problem,span=2}{
    ...
    \begin{minipage}[t]{0.42\linewidth}
        \centering\includegraphics[width=\linewidth]{q1}\\[-0.2em]
        {\small $\mathbf{Q}$ (Wan2.2)}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.42\linewidth}
        \centering\includegraphics[width=\linewidth]{k1}\\[-0.2em]
...
\headerbox{\bf\color{blue} Learned Rotation Must Be Orthogonal}{name=ortho,column=2,below=results,span=3}{
    \textbf{\color{blue}Wan2.2 I2V.} Learned rotations improve generated frames only under the $\mathrm{O}(2)$ orthogonal constraint; unconstrained rotations amplify quantization error and visibly degrade the output.
    \begin{center}
    {\scriptsize
